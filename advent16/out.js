...
(function dartProgram() {
  ...
  var A = {JS_CONST: function JS_CONST() {
    ...
    parse() {
      var label, flow_rate, neighbors, ne, done, t2,
        list = A.LinkedHashMap_LinkedHashMap$_empty(type$.String, type$.Valve),
        line = A.stdin().readLineSync$0(),
        t1 = type$.JSArray_String;
      while (true) {
        label = line.substring$2(line, 6, 8);
        flow_rate = A.int_parse(line.substring$2(line, line.indexOf$1(line, "=").$add(0, 1), line.indexOf$1(line, ";")));
        neighbors = A._setArrayType([], t1);
        ne = line.substring$1(0, line.indexOf$1(line, "to v").$add(0, 5));
        A.printString(A.S(ne));
        ne = ne.substring$1(0, ne.indexOf$1(0, " ").$add(0, 1));
        A.printString(A.S(ne));
        if (!ne.contains$1(0, ",")) {
          B.JSArray_methods.add$1(neighbors, ne.substring$2(0, 0, 2));
          done = true;
        } else
          done = false;
        for (; !done;) {
          ...
          if (!done) {
            B.JSArray_methods.add$1(neighbors, ne.substring$2(0, 0, 2));
            ne = ne.substring$1(0, ne.indexOf$1(0, ",").$add(0, 2));
          }
        }
        A.printString(A.S(label));
        list.$indexSet(0, label, new A.Valve(label, flow_rate, neighbors));
        $.$get$_ioOverridesToken();
        t2 = $.$get$_stdin();
        line = t2.readLineSync$0();
      }
      return new A.Valves(list);
    },
    main() {
      var t3, x, _i,
        vs = A.parse(),
      ...
      A.print(1);
      t2 = vs.cur;
      A.print(t2 == null ? null : t2.flow_rate);
      for (t2 = A.LinkedHashMapKeyIterator$(t1, t1._modifications, A._instanceType(t1)._precomputed1); t2.moveNext$0();) {
        t3 = t2._current;
        A.printString(t3);
        x = t1.$index(0, t3);
        if (x != null)
          for (t3 = x.neighbors.length, _i = 0; _i < t3; ++_i)
            ;
      }
    },
    Valve: function Valve(t0, t1, t2) {
      this.label = t0;
      this.flow_rate = t1;
      this.neighbors = t2;
    },
    Valves: function Valves(t0) {
      this.list = t0;
      this.cur = null;
    },
    printString(string) {
  ...
  A.FormatException.prototype = {
    toString$0(_) {
      var t1 = A.S(this.message);
      return "FormatException: " + t1;
    }
  };
  ...
  (function inheritance() {
    ...
    _inheritMany(A.Object, [A.JS_CONST, J.Interceptor, J.ArrayIterator, A.Error, A.Iterable, A.Closure, A.MapMixin, A.LinkedHashMapCell, A.LinkedHashMapKeyIterator, A.Rti, A._FunctionParameters, A.FormatException, A.Null, A.StringBuffer, A.Valve, A.Valves]);
  ...
  A._Universe_addRules(init.typeUniverse, JSON.parse('{"JSBool":{"bool":[]},"JSArray":{"List":["1"],"Iterable":["1"]},"JSUnmodifiableArray":{"JSArray":["1"],"List":["1"],"Iterable":["1"]},"JSNumber":{"num":[]},"JSInt":{"int":[],"num":[]},"JSNumNotInt":{"num":[]},"JSString":{"String":[],"Pattern":[]},"EfficientLengthIterable":{"Iterable":["1"]},"JsLinkedHashMap":{"MapMixin":["1","2"],"Map":["1","2"]},"LinkedHashMapKeyIterable":{"Iterable":["1"]},"MapBase":{"MapMixin":["1","2"],"Map":["1","2"]},"MapMixin":{"Map":["1","2"]},"String":{"Pattern":[]}}'));
  A._Universe_addErasedTypes(init.typeUniverse, JSON.parse('{"EfficientLengthIterable":1,"MapBase":2,"Iterable":1}'));
